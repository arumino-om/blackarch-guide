...
\hypersetup{
  pdftitle={BlackArch Linux, BlackArch Linux ガイド},
  pdfsubject={BlackArch Linux, BlackArch Linux ガイド},
...
\end{figure}
\vspace{0.5cm}
\Huge{\textbf{BlackArch Linux ガイド}}\\
\vspace{1cm}
...
\section{概要}
BlackArch Linux ガイドは、いくつかのパートで構成されています。
\begin{itemize}
\item はじめに - 大まかな概要と導入、および追加で役立つプロジェクトの情報
\item ユーザーガイド - 一般的なユーザーがBlackArchを効果的に使用するために知っておくべきこと
\item 開発者ガイド - BlackArchの開発に参加・貢献する方法
\item ツールガイド - ツールの詳細と使用例(WIP)
\end{itemize}

\section{BlackArch Linuxとは？}
BlackArchは、ペネトレーションテスターとセキュリティ研究者のための包括的なLinuxディストリビューションです。

このツールセットはArch Linuxの\href{https://wiki.archlinux.org/index.php/Unofficial\_User\_Repositories}
{非公式ユーザーリポジトリ}として配布されているため、既存のArch Linux上に直接BlackArchをインストールすることができます。
パッケージは個別にインストールすることも、カテゴリごとにインストールすることも可能です。

常に拡張を続けるリポジトリには、現在\href{https://www.blackarch.org/tools.html}{2600}個を超えるツールが含まれています。
リポジトリの品質を保つため、すべてのツールはコードベースに追加される前に徹底的にテストされています。
% should quickly describe the testing methods/code review procedures etc

\section{BlackArch Linux の歴史}
近日公開...

\section{サポートされているプラットフォーム}
近日公開...

\section{チームに参加する}
次の方法で、BlackArchチームに連絡できます。

ウェブサイト: \url{https://www.blackarch.org/}

メール: \href{mailto:team@blackarch.org}{team@blackarch.org}
...
\section{インストール}
このセクションでは、BlackArchリポジトリのセットアップ方法やパッケージのインストール方法について説明します。
BlackArchは、リポジトリからバイナリパッケージをインストールする方法と、ソースからコンパイルしてインストールする方法の両方をサポートしています。

BlackArchは通常のArchインストールと互換性があり、非公式のユーザーリポジトリとして機能します。
代わりにISOが必要な場合は、\href{https://www.blackarch.org/downloads.html#iso}{ISOs}セクションをご覧ください。
% 訳注: ISOs は公式サイト上での記述（

\subsection{ArchLinux上にインストール}
\href{https://blackarch.org/strap.sh}{strap.sh}をrootで実行し、案内に従ってください。
以下に例を示します。
\begin{lstlisting}
   curl -O https://blackarch.org/strap.sh
   sha1sum strap.sh # 5ea40d49ecd14c2e024deecf90605426db97ea0c と合致するはずです
   sudo chmod +x strap.sh
   sudo ./strap.sh
\end{lstlisting}

次に、マスターパッケージリストの新しいコピーをダウンロードし、パッケージを同期します。
\begin{lstlisting}
  sudo pacman -Syyu
\end{lstlisting}


\subsection{パッケージをインストール}
これで、blackarchリポジトリからツールをインストールできるようになりました。

\begin{enumerate}
\item 利用可能なすべてのツールをリストアップするには、以下を実行します
\begin{lstlisting}
  pacman -Sgg | grep blackarch | cut -d' ' -f2 | sort -u
\end{lstlisting}

\item すべてのツールをインストールするには、以下を実行します
\begin{lstlisting}
  pacman -S blackarch
\end{lstlisting}

\item カテゴリ内のツールをインストールするには、以下を実行します
\begin{lstlisting}
  pacman -S blackarch-<category>
\end{lstlisting}

\item blackarchのカテゴリを閲覧するには、以下を実行します
\begin{lstlisting}
...
\subsection{ソースからパッケージをインストール}
別のインストール方法として、BlackArchパッケージをソースからビルドすることもできます。
PKGBUILDは\href{https://github.com/BlackArch/blackarch/tree/master/packages}{github}で入手できます。
リポジトリ全体をビルドする場合は、\href{https://github.com/BlackArch/blackman}{Blackman}を使用できます。

\begin{itemize}
\item 最初に、Blackmanをインストールする必要があります。BlackArchパッケージリポジトリがすでに設定済みなら、Blackmanをインストールできます
\begin{lstlisting}
  pacman -S blackman
\end{lstlisting}

\item Blackmanをソースからビルドしてインストールできます
\begin{lstlisting}
  mkdir blackman
  cd blackman
  wget https://raw.github.com/BlackArch/blackarch/master/packages/blackman/PKGBUILD
  # PKGBUILDが悪意を持って改ざんされていないことを確認してください
  makepkg -s
\end{lstlisting}

\item または、AURからBlackmanをインストールできます
\begin{lstlisting}
  <好きなAURヘルパー> -S blackman
\end{lstlisting}

\end{itemize}

\subsection{Blackmanの基本的な使い方} 
Blackmanの使い方は非常に簡単ですが、フラグは一般的にpacmanのようなコマンドで期待されるものとは異なります。
基本的な使い方を以下に概説します。
\begin{itemize}
\item パッケージをダウンロード、コンパイルしてインストール
\begin{lstlisting}
  sudo blackman -i package
\end{lstlisting}

\item カテゴリ全体をダウンロード、コンパイルしてインストール
\begin{lstlisting}
  sudo blackman -g group
\end{lstlisting}

\item すべてのBlackArchツールをダウンロード、コンパイルしてインストール
\begin{lstlisting}
  sudo blackman -a
\end{lstlisting}

\item blackarchカテゴリをリストアップ
\begin{lstlisting}
  blackman -l
\end{lstlisting}

\item カテゴリ内のツールをリストアップ
\begin{lstlisting}
  blackman -p category
\end{lstlisting}
...
\subsection{full-, netinstall- ISO またはArchLinuxからインストール}
full- or netinstall-ISOからBlackArch Linuxをインストールすることができます。
\\詳細は\url{https://www.blackarch.org/download.html#iso}をご覧ください。
以下のステップはISOが起動した後に行ってください。

\begin{itemize}
\item blackarch-installerパッケージをインストール
\begin{lstlisting}
