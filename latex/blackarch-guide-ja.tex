...
\section{概要}
BlackArch Linux ガイドは、いくつかのパートに分かれています。
\begin{itemize}
\item はじめに - 大まかな概要と導入、および追加で役立つプロジェクトの情報
\item ユーザーガイド - BlackArchを効果的に使用するために一般ユーザーが知っておくべきこと
\item 開発者ガイド - BlackArchの開発に参加・貢献する方法
\item ツールガイド - ツールの詳細と使用例(WIP)
\end{itemize}

\section{BlackArch Linuxとは？}
BlackArchは、ペネトレーションテスターとセキュリティ研究者のための完全なLinuxディストリビューションです。

このツールセットはArch Linuxの\href{https://wiki.archlinux.org/index.php/Unofficial\_User\_Repositories}
{非公式ユーザーリポジトリ}として配布されているため、既存のArch Linux上にBlackArchをインストールすることができます。
パッケージは個別にインストールすることも、カテゴリごとにインストールすることも可能です。

継続的に拡張されるリポジトリには、現在\href{https://www.blackarch.org/tools.html}{2600}個以上のツールが含まれています。
リポジトリの品質を保つため、すべてのツールはコードベースに追加される前に徹底的にテストされます。
...
\href{https://www.blackarch.org/downloads.html#iso}{ISOs} section.
